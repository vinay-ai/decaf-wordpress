...
use Traversable;
use Google\Web_Stories_Dependencies\WP_Error;
use Google\Web_Stories_Dependencies\WP_Http;
/**
 ...
 * @internal
 */
final class WpHttpRemoteGetRequest implements \Google\Web_Stories_Dependencies\AmpProject\RemoteGetRequest
{
    /**
    ...
    public function get($url)
    {
        ...
        do {
            ...
            if ($response instanceof \WP_Error) {
                return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse($response->get_error_message(), [], 500);
            }
            if (!isset($response['response']['code'])) {
                return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse(isset($response['response']['message']) ? $response['response']['message'] : 'Unknown error', [], 500);
            }
            ...
            if ($status < 200 || $status >= 300) {
                if (!$retries_left || \in_array($status, self::RETRYABLE_STATUS_CODES, \true) === \false) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetFromRemoteUrl::withHttpStatus($url, $status);
                }
                continue;
            }
            ...
            return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse($response['body'], $headers, (int) $status);
        } while ($retries_left--);
