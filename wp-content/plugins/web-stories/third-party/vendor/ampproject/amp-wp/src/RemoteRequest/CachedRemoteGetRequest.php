 ...
 * @internal
 */
final class CachedRemoteGetRequest implements \Google\Web_Stories_Dependencies\AmpProject\RemoteGetRequest
{
    /**
     ...
     *                                            to true.
     */
    public function __construct(\Google\Web_Stories_Dependencies\AmpProject\RemoteGetRequest $remote_request, $expiry = MONTH_IN_SECONDS, $min_expiry = DAY_IN_SECONDS, $use_cache_control = \true)
    {
    ...
    public function get($url)
    {
        ...
        if (\false !== $cached_response) {
            if (\PHP_MAJOR_VERSION >= 7) {
                $cached_response = \unserialize($cached_response, [\Google\Web_Stories_Dependencies\AmpProject\AmpWP\RemoteRequest\CachedResponse::class, \DateTimeImmutable::class]);
        ...
        if (!$cached_response instanceof \Google\Web_Stories_Dependencies\AmpProject\AmpWP\RemoteRequest\CachedResponse || $cached_response->is_expired()) {
            try {
            ...
            } catch (\Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetFromRemoteUrl $exception) {
                $status = $exception->getStatusCode();
                $expiry = new \DateTimeImmutable("+ {$this->min_expiry} seconds");
                $body = $exception->getMessage();
            }
            $cached_response = new \Google\Web_Stories_Dependencies\AmpProject\AmpWP\RemoteRequest\CachedResponse($body, $headers, $status, $expiry);
        ...
        if (!$cached_response->is_valid()) {
            throw new \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetCachedResponse($url);
        }
        return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse($cached_response->get_body(), $cached_response->get_headers(), $cached_response->get_status_code());
    }
    /**
     ...
     */
    private function get_expiry_time(\Google\Web_Stories_Dependencies\AmpProject\Response $response)
    {
