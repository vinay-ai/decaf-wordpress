...
class AMP_Image_Dimension_Extractor
{
    ...
    public static function normalize_url($url)
    {
        ...
        $normalized_url = $url;
        if (0 === \strpos($url, '//')) {
            $normalized_url = \Google\Web_Stories_Dependencies\set_url_scheme($url, 'http');
        } else {
    ...
    {
        if ($mode) {
            \Google\Web_Stories_Dependencies\_deprecated_argument(__METHOD__, 'AMP 1.1');
        }
        $transient_expiration = 30 * \Google\Web_Stories_Dependencies\DAY_IN_SECONDS;
        $urls_to_fetch = [];
        $images = [];
    ...
    private static function determine_which_images_to_fetch(&$dimensions, &$urls_to_fetch)
    {
        foreach ($dimensions as $url => $value) {
            ...
            \set_transient($transient_lock_name, 1, \Google\Web_Stories_Dependencies\MINUTE_IN_SECONDS);
        }
    }
    /**
    ...
    private static function process_fetched_images($urls_to_fetch, $images, &$dimensions, $transient_expiration)
    {
        foreach ($urls_to_fetch as $url_data) {
            ...
            \Google\Web_Stories_Dependencies\delete_transient($url_data['transient_lock_name']);
        }
    }
    /**
    ...
    {
        return 'amp-wp, v' . \Google\Web_Stories_Dependencies\AMP__VERSION . ', ' . \home_url();
    }
}
