...
class AMP_Layout_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
    ...
    public function sanitize()
    {
        ...
        foreach ($nodes as $node) {
            ...
            if ('100%' === $width) {
                ...
                $node->setAttribute('layout', \Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED_HEIGHT);
            }
        }
    ...
    private function set_layout_fill(\DOMElement $node)
    {
        ...
        if (\Google\Web_Stories_Dependencies\AmpProject\Layout::FILL !== $node->getAttribute('layout')) {
            $node->setAttribute('layout', \Google\Web_Stories_Dependencies\AmpProject\Layout::FILL);
        }
    }
}
