 ...
 */
final class PreloadHeroImageConfiguration extends \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\BaseTransformerConfiguration
{
    /**
    ...
    protected function validate($key, $value)
    {
        switch ($key) {
            case self::INLINE_STYLE_BACKUP_ATTRIBUTE:
                if (!\is_string($value)) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidConfigurationValue::forInvalidSubValueType(self::class, self::INLINE_STYLE_BACKUP_ATTRIBUTE, 'string', \is_object($value) ? \get_class($value) : \gettype($value));
                }
                break;
            case self::PRELOAD_HERO_IMAGE:
                if (!\is_bool($value)) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidConfigurationValue::forInvalidSubValueType(self::class, self::PRELOAD_HERO_IMAGE, 'boolean', \is_object($value) ? \get_class($value) : \gettype($value));
                }
                break;
            case self::PRELOAD_SRCSET:
                if (!\is_bool($value)) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidConfigurationValue::forInvalidSubValueType(self::class, self::PRELOAD_SRCSET, 'boolean', \is_object($value) ? \get_class($value) : \gettype($value));
                }
                break;
        }
        return $value;
