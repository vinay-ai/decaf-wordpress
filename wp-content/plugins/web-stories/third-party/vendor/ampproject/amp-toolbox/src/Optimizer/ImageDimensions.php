...
final class ImageDimensions
{
     ...
     */
    public function __construct(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $image)
    {
        $this->image = $image;
    }
    /**
    ...
    public function getDimensionsFromParent()
    {
        ...
        while ($element->parentNode && ++$level < 3) {
            ...
            $width = $element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH) ? $element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH) : -1;
            $height = $element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT) ? $element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT) : -1;
            if (empty($width)) {
                $width = -1;
            }
            if (empty($height)) {
            ...
            if (\Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE === $element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT)) {
    ...
    public function isTiny($threshold = self::TINY_THRESHOLD)
    {
        ...
        if (!$this->hasWidth() && !$this->hasHeight()) {
            if ($this->getLayout() === \Google\Web_Stories_Dependencies\AmpProject\Layout::FILL) {
        ...
        if (\in_array($widthUnit, \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::RELATIVE_UNITS, \true) && \in_array($heightUnit, \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::RELATIVE_UNITS, \true)) {
            return \false;
        }
        // If only one of the units is relative, compare the other against the threshold.
        if (\in_array($widthUnit, \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::RELATIVE_UNITS, \true)) {
            return \is_numeric($height) && $height < $threshold;
        } elseif (\in_array($heightUnit, \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::RELATIVE_UNITS, \true)) {
    ...
    public function getWidth()
    {
        if ($this->width === null) {
            $this->width = -1;
            $width = $this->image->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH);
            if (\trim($width) !== '') {
                if (\is_numeric($width)) {
                    $intWidth = (int) $width;
    ...
    public function getHeight()
    {
        if ($this->height === null) {
            $this->height = -1;
            $height = $this->image->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT);
            if (\trim($height) !== '') {
    ...
    public function getNumericWidth()
    {
        ...
        return \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::convertIntoPixels($width, $widthUnit);
    }
    /**
    ...
    public function getNumericHeight()
    {
        ...
        return \Google\Web_Stories_Dependencies\AmpProject\LengthUnit::convertIntoPixels($height, $heightUnit);
    }
    /**
    ...
    {
        if ($this->layout === null) {
            $this->layout = $this->image->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT) ? (string) $this->image->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT) : '';
        }
        return $this->layout;
    }
}
