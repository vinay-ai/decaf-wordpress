...
final class ServerSideRendering implements \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Transformer
{
    ...
    public function transform(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\ErrorCollection $errors)
    {
        ...
        foreach ($document->ampElements as $ampElement) {
            ...
            if (!$ampElement instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                continue;
            }
     ...
     */
    private function applyLayout(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\ErrorCollection $errors)
    {
        ...
        try {
            ...
            if ($layout && $layout === \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE) {
                $newElement->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT, $layout);
            }
        ...
        } catch (\Google\Web_Stories_Dependencies\AmpProject\Exception\MaxCssByteCountExceeded $exception) {
            $errors->add(\Google\Web_Stories_Dependencies\AmpProject\Optimizer\Error\CannotPerformServerSideRendering::fromMaxCssByteCountExceededException($exception, $element));
            return \false;
        }
        return $newElement;
    }
     ...
     * @param CssLength $height  Calculated height.
     */
    private function applyLayoutAttributes(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, $layout, \Google\Web_Stories_Dependencies\AmpProject\CssLength $width, \Google\Web_Stories_Dependencies\AmpProject\CssLength $height)
    {
     ...
     */
    private function addClass(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, $class)
    {
     ...
     */
    private function maybeAddSizerInto(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, $layout, \Google\Web_Stories_Dependencies\AmpProject\CssLength $width, \Google\Web_Stories_Dependencies\AmpProject\CssLength $height)
    {
     ...
     * @return Element
     */
    private function createResponsiveSizer(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\CssLength $width, \Google\Web_Stories_Dependencies\AmpProject\CssLength $height, $style = 'padding-top:%s%%')
    {
     ...
     */
    private function isWithinTemplate(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element)
    {
        $parent = $element->parentNode;
        while ($parent !== null) {
            if ($parent instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                if ($parent->tagName === \Google\Web_Stories_Dependencies\AmpProject\Tag::TEMPLATE) {
                    return \true;
                }
                if ($parent->tagName === \Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT && $parent->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::TEMPLATE)) {
                    return \true;
                }
            }
     ...
     */
    private function isAmpExperimentUsed(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element)
    {
        $script = null;
        $child = $element->firstChild;
        while ($child) {
            if ($child instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element && $child->tagName === \Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT && \strtolower($child->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::TYPE)) === \Google\Web_Stories_Dependencies\AmpProject\Attribute::TYPE_JSON) {
                $script = $child;
                break;
            }
     ...
     */
    private function adaptBlockingAttributes(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $ampElement, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\ErrorCollection $errors)
    {
     ...
     */
    private function extractSizesAttributeCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, \DOMAttr $attribute)
    {
     ...
     * @return CssRule[] Extract custom CSS styling.
     */
    private function extractHeightsAttributeCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, \DOMAttr $attribute)
    {
     ...
     */
    private function extractAttributeCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, \DOMAttr $attribute, $mainStyle, $mediaQueryStyle)
    {
        if (empty($attribute->nodeValue)) {
            return [];
        }
        $sourceSizes = \explode(',', $attribute->nodeValue);
        $lastItem = \trim(\array_pop($sourceSizes), \Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule::CSS_TRIM_CHARACTERS);
        if (empty($lastItem)) {
        ...
        foreach (\array_reverse($sourceSizes) as $sourceSize) {
            ...
            $mediaCondition = \trim($matches['media_condition'], \Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule::CSS_TRIM_CHARACTERS);
            if (empty($mediaCondition)) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidHtmlAttribute::fromAttribute($attribute->nodeName, $element);
            }
            $dimension = \trim($matches['dimension'], \Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule::CSS_TRIM_CHARACTERS);
            if (empty($dimension)) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidHtmlAttribute::fromAttribute($attribute->nodeName, $element);
            }
     ...
     * @return CssRule[] Extract custom CSS styling.
     */
    private function extractMediaAttributeCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, \DOMAttr $attribute)
    {
        $attributeValue = \trim($attribute->nodeValue, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule::CSS_TRIM_CHARACTERS);
     ...
     */
    private function isExcludedElement(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element)
    {
