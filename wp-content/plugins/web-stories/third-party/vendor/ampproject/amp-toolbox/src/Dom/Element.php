...
final class Element extends \DOMElement
{
    ...
    public function addInlineStyle($style)
    {
        ...
        $existingStyle = (string) \trim($this->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::STYLE));
        if (!empty($existingStyle)) {
            $existingStyle = \rtrim($existingStyle, ';') . ';';
        }
        $newStyle = $existingStyle . $style;
        return $this->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::STYLE, $newStyle);
    }
    /**
    ...
    {
        if ($name === \Google\Web_Stories_Dependencies\AmpProject\Attribute::STYLE && $this->ownerDocument->isCssMaxByteCountEnforced()) {
            $newByteCount = \strlen($value);
            if ($this->ownerDocument->getRemainingCustomCssSpace() < $newByteCount - $this->inlineStyleByteCount) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Exception\MaxCssByteCountExceeded::forInlineStyle($this, $value);
            }
            $this->ownerDocument->addInlineStyleByteCount($newByteCount - $this->inlineStyleByteCount);
            $this->inlineStyleByteCount = $newByteCount;
            return parent::setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::STYLE, $value);
        }
     ...
     */
    public function copyAttributes($attributes, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $target, $defaultSeparator = ',')
    {
        foreach ((array) $attributes as $attribute) {
            if ($this->hasAttribute($attribute)) {
                ...
                if ($target->hasAttribute($attribute)) {
                    switch ($attribute) {
                        case \Google\Web_Stories_Dependencies\AmpProject\Attribute::ON:
                            $values = self::mergeAmpActions($target->getAttribute($attribute), $values);
                            break;
                        case \Google\Web_Stories_Dependencies\AmpProject\Attribute::CLASS_:
    ...
    public function addAmpAction($event, $action)
    {
        ...
        if (!$this->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ON)) {
            // There's no "on" attribute yet, so just add it and be done.
            $this->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ON, $eventActionString);
            return;
        }
        $this->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ON, self::mergeAmpActions($this->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ON), $eventActionString));
    }
    /**
        ...
        switch ($name) {
            case 'inlineStyleByteCount':
                if (!isset($this->inlineStyleByteCount)) {
                    $this->inlineStyleByteCount = \strlen((string) $this->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::STYLE));
                }
