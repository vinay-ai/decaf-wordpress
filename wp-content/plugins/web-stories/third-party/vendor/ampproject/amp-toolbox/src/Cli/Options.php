...
class Options
{
     ...
     */
    public function __construct(\Google\Web_Stories_Dependencies\AmpProject\Cli\Colors $colors = null)
    {
    ...
    public function registerArgument($name, $help, $required = \true, $command = '')
    {
        if (!isset($this->setup[$command])) {
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidCommand::forUnregisteredCommand($command);
        }
    ...
    public function registerCommand($name, $help)
    {
        if (isset($this->setup[$name])) {
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidCommand::forAlreadyRegisteredCommand($name);
        }
    ...
    public function registerOption($long, $help, $short = null, $needsArgument = \false, $command = '')
    {
        ...
        if ($short) {
            if (\strlen($short) > 1) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidArgument::forMultiCharacterShortOption();
            }
    ...
    public function checkArguments()
    {
        ...
        if ($required > $argumentCount) {
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\MissingArgument::forNotEnoughArguments();
        }
    }
    /**
        ...
        for ($index = 0; $index < $argumentCount; $index++) {
            ...
            if (\strlen($argument) > 1 && $argument[1] === '-') {
                ...
                if (!isset($this->setup[$this->command]['options'][$option])) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidOption::forUnknownOption($option);
                }
                // Argument required?
                if ($this->setup[$this->command]['options'][$option]['needsArgument']) {
                    ...
                    if (\is_null($value)) {
                        throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\MissingArgument::forNoArgument($option);
                    }
    ...
    {
        $tableFormatter = new \Google\Web_Stories_Dependencies\AmpProject\Cli\TableFormatter($this->colors);
        $text = '';
        ...
        foreach ($this->setup as $command => $config) {
            ...
            if (!$command) {
                $text .= $this->colors->wrap('USAGE:', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_BROWN);
                $text .= "\n";
                $text .= '   ' . $this->bin;
                $indentation = 2;
            } else {
                $text .= $this->newline;
                $text .= $this->colors->wrap('   ' . $command, \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_PURPLE);
                $indentation = 4;
            }
            if ($hasOptions) {
                $text .= ' ' . $this->colors->wrap('<OPTIONS>', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_GREEN);
            }
            if (!$command && $hasCommands) {
                $text .= ' ' . $this->colors->wrap('<COMMAND> ...', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_PURPLE);
            }
            foreach ($this->setup[$command]['arguments'] as $argument) {
                $output = $this->colors->wrap('<' . $argument['name'] . '>', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_CYAN);
                if (!$argument['required']) {
                    $output = '[' . $output . ']';
                }
                $text .= ' ' . $output;
            ...
            if ($hasOptions) {
                if (!$command) {
                    $text .= "\n";
                    $text .= $this->colors->wrap('OPTIONS:', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_BROWN);
                }
                $text .= "\n";
            ...
            if ($hasArguments) {
                if (!$command) {
                    $text .= "\n";
                    $text .= $this->colors->wrap('ARGUMENTS:', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_BROWN);
                }
                $text .= $this->newline;
                foreach ($this->setup[$command]['arguments'] as $argument) {
                    $name = '<' . $argument['name'] . '>';
            ...
            if (!$command && $hasCommands) {
                $text .= "\n";
                $text .= $this->colors->wrap('COMMANDS:', \Google\Web_Stories_Dependencies\AmpProject\Cli\Colors::C_BROWN);
                $text .= "\n";
    ...
    private function readPHPArgv()
    {
        ...
        throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidArgument::forUnreadableArguments();
    }
}
