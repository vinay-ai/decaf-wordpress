...
class TableFormatter
{
    ...
    protected function calculateColumnWidths($columns)
    {
        ...
        foreach ($columns as $index => $column) {
            ...
            if ($column === '*') {
                ...
                if ($fluid < 0) {
                    ...
                    throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidColumnFormat::forMultipleFluidColumns();
                }
            }
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidColumnFormat::forUnknownColumnFormat($column);
        }
        $allocated = $fixed;
        ...
        $remain = $this->maxWidth - $allocated;
        if ($remain < 0) {
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\Cli\InvalidColumnFormat::forExceededMaxWidth();
        }
