...
final class CannotInlineRuntimeCss implements \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Error
{
     ...
     * @return self
     */
    public static function fromException(\Exception $exception, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element $ampRuntimeStyle, $version)
    {
        $version = empty($version) ? 'unspecified version' : "version {$version}";
        return new self(\sprintf(self::EXCEPTION_STRING, $exception, new \Google\Web_Stories_Dependencies\AmpProject\Dom\ElementDump($ampRuntimeStyle), $version));
    }
    /**
