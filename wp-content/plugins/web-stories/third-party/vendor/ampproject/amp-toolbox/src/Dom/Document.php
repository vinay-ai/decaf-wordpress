...
final class Document extends \DOMDocument
{
     ...
     * @var string
     */
    const AMP_EMOJI_ATTRIBUTE_PATTERN = '/<html\\s([^>]*?(?:' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI_ALT . '|' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI . ')(4(?:ads|email))?[^>]*?)>/i';
    ...
    {
        $this->originalEncoding = (string) $encoding ?: \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::UNKNOWN;
        parent::__construct($version ?: '1.0', \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP);
        $this->registerNodeClass(\DOMElement::class, \Google\Web_Stories_Dependencies\AmpProject\Dom\Element::class);
        $this->options = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::DEFAULTS;
    }
    /**
    ...
    public static function fromHtml($html, $options = [])
    {
        ...
        if (\is_string($options)) {
            $options = [\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::ENCODING => $options];
        }
        $encoding = isset($options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::ENCODING]) ? $options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::ENCODING] : null;
    ...
    public function loadHTMLFragment($source, $options = [])
    {
        ...
        if (\is_int($options)) {
            $options = [\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::LIBXML_FLAGS => $options];
        }
        ...
        if (\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP !== \strtolower($this->originalEncoding)) {
        ...
        $this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::LIBXML_FLAGS] |= \LIBXML_COMPACT;
        /*
        ...
        if (\defined('LIBXML_HTML_NODEFDTD')) {
            $this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::LIBXML_FLAGS] |= \constant('LIBXML_HTML_NODEFDTD');
        }
        $success = parent::loadHTML($source, $this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::LIBXML_FLAGS]);
        \libxml_clear_errors();
        ...
        if ($success) {
            ...
            if ($meta instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element && \Google\Web_Stories_Dependencies\AmpProject\Tag::META === $meta->tagName && self::HTML_HTTP_EQUIV_VALUE === $meta->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HTTP_EQUIV) && self::HTML_HTTP_EQUIV_CONTENT_VALUE === $meta->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT)) {
    ...
    private function insertMissingCharset()
    {
        ...
        $charset = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::META);
        $charset->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET, \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP);
        $this->head->insertBefore($charset, $this->head->firstChild);
    }
    /**
    ...
    public function normalizeDomStructure()
    {
        ...
        if (\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML !== $this->documentElement->nodeName) {
            ...
            if ($oldDocumentElement->nodeName === \Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD) {
                ...
                if (!$head) {
                    $head = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD);
                }
            }
            if (!$head instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToRetrieveRequiredDomElement::forHeadElement($head);
            }
            $this->head = $head;
            ...
            if ($oldDocumentElement->nodeName === \Google\Web_Stories_Dependencies\AmpProject\Tag::BODY) {
                ...
                if (!$body) {
                    $body = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY);
                }
            }
            if (!$body instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToRetrieveRequiredDomElement::forBodyElement($body);
            }
            $this->body = $body;
    ...
    private function movePostBodyNodesToBody()
    {
        ...
        while ($this->documentElement->nextSibling) {
            $nextSibling = $this->documentElement->nextSibling;
            if ($nextSibling instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element && \Google\Web_Stories_Dependencies\AmpProject\Tag::HTML === $nextSibling->nodeName) {
    ...
    {
        if ($this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX] === \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX_DATA_ATTRIBUTE) {
            // All amp-bind attributes should remain in their converted data attribute form.
            return $html;
        }
        if ($this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX] === \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX_AUTO && empty($this->convertedAmpBindAttributes)) {
        ...
        $replaceCallback = function ($tagMatches) {
            ...
            while (\preg_match(self::AMP_BIND_DATA_ATTRIBUTE_ATTR_PATTERN, \substr($oldAttrs, $offset), $attrMatches)) {
                ...
                if ($this->options[\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX] === \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Option::AMP_BIND_SYNTAX_SQUARE_BRACKETS || \in_array($attrName, $this->convertedAmpBindAttributes, \true)) {
    ...
    private function adaptEncoding($source)
    {
        ...
        if (\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::UNKNOWN === $this->originalEncoding && \function_exists('mb_detect_encoding')) {
            $this->originalEncoding = \mb_detect_encoding($source, \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::DETECTION_ORDER, \true);
        }
        ...
        if (empty($this->originalEncoding)) {
            $this->originalEncoding = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP;
            // @codeCoverageIgnore
        }
        ...
        if (\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::UNKNOWN === $this->originalEncoding && \function_exists('mb_detect_encoding')) {
            $detectedEncoding = \mb_detect_encoding($source, \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::DETECTION_ORDER, \true);
        ...
        $target = \false;
        if (\Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP !== \strtolower($this->originalEncoding)) {
            $target = \function_exists('mb_convert_encoding') ? \mb_convert_encoding($source, \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP, $this->originalEncoding) : \false;
        }
    ...
    private function detectAndStripEncoding($content)
    {
        ...
        if ($charsetTag) {
            ...
            if (\strtolower($encoding) !== \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP) {
    ...
    private function sanitizeEncoding($encoding)
    {
        ...
        if ($encoding === \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::AMP) {
            return $encoding;
        }
        ...
        if (\array_key_exists($encoding, \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::MAPPINGS)) {
            $encoding = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::MAPPINGS[$encoding];
        }
        if (!\in_array($encoding, $knownEncodings, \true)) {
            return \Google\Web_Stories_Dependencies\AmpProject\Dom\Document\Encoding::UNKNOWN;
        }
        return $encoding;
    }
    /**
    ...
    private function convertAmpEmojiAttribute($source)
    {
        ...
        return \preg_replace_callback(self::AMP_EMOJI_ATTRIBUTE_PATTERN, function ($matches) {
            ...
            foreach ($attributes as $index => $attribute) {
                $attributeMatches = [];
                if (\preg_match('/^(' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI_ALT . '|' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI . ')(4(?:ads|email))?$/i', $attribute, $attributeMatches)) {
    ...
    {
        return $node instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element && \in_array($node->nodeName, \Google\Web_Stories_Dependencies\AmpProject\Tag::ELEMENTS_ALLOWED_IN_HEAD, \true) || $node instanceof \DOMText && \preg_match('/^\\s*$/', $node->nodeValue) || $node instanceof \DOMComment;
    }
    /**
     ...
     */
    public function getElementId(\Google\Web_Stories_Dependencies\AmpProject\Dom\Element $element, $prefix = 'i-amp')
    {
        if ($element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ID)) {
            return $element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ID);
        }
        $id = $this->getUniqueId($prefix);
        while ($this->getElementById($id) instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
            $id = $this->getUniqueId($prefix);
        }
        $element->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::ID, $id);
        return $id;
    }
    /**
    ...
    {
        $style = \trim($style, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule::CSS_TRIM_CHARACTERS);
        ...
        if ($this->getRemainingCustomCssSpace() < $newByteCount - $this->ampCustomStyleByteCount) {
            throw \Google\Web_Stories_Dependencies\AmpProject\Exception\MaxCssByteCountExceeded::forAmpCustom($newStyle);
        }
    ...
    public function __get($name)
    {
        switch ($name) {
            ...
            case \Google\Web_Stories_Dependencies\AmpProject\Tag::HTML:
                $html = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML)->item(0);
                if ($html === null) {
                    // Document was assembled manually and bypassed normalisation.
                    $this->normalizeDomStructure();
                    $html = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML)->item(0);
                }
                if (!$html instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToRetrieveRequiredDomElement::forHtmlElement($html);
                }
                $this->html = $html;
                return $this->html;
            ...
            case \Google\Web_Stories_Dependencies\AmpProject\Tag::BODY:
                ...
                $this->body = $body;
                return $this->body;
            case \Google\Web_Stories_Dependencies\AmpProject\Attribute::VIEWPORT:
                ...
                for ($node = $this->head->firstChild; $node !== null; $node = $node->nextSibling) {
                    if ($node instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element && $node->tagName === \Google\Web_Stories_Dependencies\AmpProject\Tag::META && $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::NAME) === \Google\Web_Stories_Dependencies\AmpProject\Attribute::VIEWPORT) {
                        return $node;
                    }
                }
                return null;
            ...
            case 'ampCustomStyle':
                ...
                if (!$ampCustomStyle instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Element) {
                    $ampCustomStyle = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::STYLE);
                    $ampCustomStyle->appendChild($this->createAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_CUSTOM));
     ...
     */
    public function enforceCssMaxByteCount($maxByteCount = \Google\Web_Stories_Dependencies\AmpProject\Amp::MAX_CSS_BYTE_COUNT)
    {
