...
class Resolver
{
    ...
    public function __construct(\Google\Web_Stories_Dependencies\enshrined\svgSanitize\data\XPath $xPath, $useNestingLimit)
    {
        $this->xPath = $xPath;
    ...
    public function findByElement(\DOMElement $element, $considerChildren = \false)
    {
        foreach ($this->subjects as $subject) {
            if ($element === $subject->getElement() || $considerChildren && \Google\Web_Stories_Dependencies\enshrined\svgSanitize\Helper::isElementContainedIn($element, $subject->getElement())) {
                return $subject;
            }
        }
        return null;
    ...
    {
        return \array_filter($this->subjects, function (\Google\Web_Stories_Dependencies\enshrined\svgSanitize\ElementReference\Subject $subject) use($elementId) {
    ...
    protected function collectIdentifiedElements()
    {
        ...
        foreach ($elements as $element) {
            $this->subjects[$element->getAttribute('id')] = new \Google\Web_Stories_Dependencies\enshrined\svgSanitize\ElementReference\Subject($element, $this->useNestingLimit);
        }
    }
    /**
    ...
    protected function processReferences()
    {
        ...
        foreach ($this->subjects as $subject) {
            ...
            foreach ($useElements as $useElement) {
                $useId = \Google\Web_Stories_Dependencies\enshrined\svgSanitize\Helper::extractIdReferenceFromHref(\Google\Web_Stories_Dependencies\enshrined\svgSanitize\Helper::getElementHref($useElement));
    ...
    protected function determineInvalidSubjects()
    {
        foreach ($this->subjects as $subject) {
            ...
            $useId = \Google\Web_Stories_Dependencies\enshrined\svgSanitize\Helper::extractIdReferenceFromHref(\Google\Web_Stories_Dependencies\enshrined\svgSanitize\Helper::getElementHref($subject->getElement()));
            try {
            ...
            } catch (\Google\Web_Stories_Dependencies\enshrined\svgSanitize\Exceptions\NestingException $e) {
     ...
     */
    protected function markSubjectAsInvalid(\Google\Web_Stories_Dependencies\enshrined\svgSanitize\ElementReference\Subject $subject)
    {
