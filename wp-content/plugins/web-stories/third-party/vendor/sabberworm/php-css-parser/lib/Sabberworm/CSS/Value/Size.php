...
class Size extends \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\PrimitiveValue
{
    ...
    public static function parse(\Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\ParserState $oParserState, $bIsColorComponent = \false)
    {
        $sSize = '';
        ...
        if (\preg_match('/^(%|[a-zA-Z0-9]+)/', $oParserState->peek($iMaxSizeUnitLength), $matches)) {
            ...
            if (isset($aSizeUnits[$iUnitLength][$sUnit])) {
                ...
                throw new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\UnexpectedTokenException('Unit', $sUnit, 'identifier', $oParserState->currentLine());
            }
        }
        return new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size(\floatval($sSize), $sUnit, $bIsColorComponent, $oParserState->currentLine());
    }
    private static function getSizeUnits()
    {
        if (self::$SIZE_UNITS === null) {
            self::$SIZE_UNITS = array();
            foreach (\explode('/', \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size::ABSOLUTE_SIZE_UNITS . '/' . \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size::RELATIVE_SIZE_UNITS . '/' . \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size::NON_SIZE_UNITS) as $val) {
                $iSize = \strlen($val);
