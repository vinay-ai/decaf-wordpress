...
class FasterImage
{
    ...
    {
        $stream = new \Google\Web_Stories_Dependencies\WillWashburn\Stream\Stream();
        $parser = new \Google\Web_Stories_Dependencies\FasterImage\ImageParser($stream);
        $result['rounds'] = 0;
        $result['bytes'] = 0;
        $result['size'] = 'failed';
        $ch = \curl_init();
        ...
        \curl_setopt($ch, \CURLOPT_WRITEFUNCTION, function ($ch, $str) use(&$result, &$parser, &$stream, $url) {
            ...
            try {
            ...
            } catch (\Google\Web_Stories_Dependencies\WillWashburn\Stream\Exception\StreamBufferTooSmallException $e) {
                /*
                ...
                $result['size'] = 'failed';
                return \strlen($str);
            } catch (\Google\Web_Stories_Dependencies\FasterImage\Exception\InvalidImageException $e) {
                /*
