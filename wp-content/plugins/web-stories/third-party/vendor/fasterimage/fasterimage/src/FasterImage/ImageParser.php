...
class ImageParser
{
     ...
     */
    public function __construct(\Google\Web_Stories_Dependencies\WillWashburn\Stream\StreamableInterface &$stream)
    {
        $this->stream = $stream;
    }
    /**
                ...
                case 'sof':
                    ...
                    if ($b === 0xe1) {
                        ...
                        if ($stream->read(4) === 'Exif') {
                            ...
                            // any exceptions here
                            try {
                                $exif = new \Google\Web_Stories_Dependencies\FasterImage\ExifParser($stream);
                            } catch (\Exception $e) {
                            }
                        }
                        break;
    ...
    {
        $exif = new \Google\Web_Stories_Dependencies\FasterImage\ExifParser($this->stream);
        if ($exif->isRotated()) {
